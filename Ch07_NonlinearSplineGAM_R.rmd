...
pacman::p_load(ISLR
               ...
               ,mboost
               ,rpart
               ,mgcv # choose between gam and mgcv
               ,wordcloud
               ,knitr,TTR,zoo
               ,leaps # regsubsets
               ,reshape
...
colSums(Train_COVID==98)
colSums(Test_COVID==98)
unique(Train_COVID$DIABETES)
## Recode missing data as NA #
Train_COVID$ASTHMA  <- ifelse( Train_COVID$ASTHMA  <3, Train_COVID$ASTHMA, NA)
Train_COVID$DIABETES<- ifelse( Train_COVID$DIABETES<3, Train_COVID$DIABETES, NA)
Train_COVID$COPD <- ifelse( Train_COVID$COPD <3, Train_COVID$COPD, NA)
Train_COVID$RENAL_CHRONIC      <- ifelse( Train_COVID$RENAL_CHRONIC <3,  Train_COVID$RENAL_CHRONIC, NA)
Train_COVID$USMER <- ifelse(Train_COVID$USMER < 3, Train_COVID$USMER, NA)
Train_COVID$CARDIOVASCULAR <- ifelse(Train_COVID$CARDIOVASCULAR < 3, Train_COVID$CARDIOVASCULAR, NA)
Train_COVID$HIPERTENSION <- ifelse(Train_COVID$HIPERTENSION < 3, Train_COVID$HIPERTENSION, NA)
Train_COVID$INMSUPR <- ifelse(Train_COVID$INMSUPR < 3, Train_COVID$INMSUPR, NA)
...
#is.factor(Train_COVID$PNEUMONIA)
# previous model: 
# glm(SURVIVED_bin ~ AGE_TRUNK + SEX + ASTHMA + DIABETES +
#                       RENAL_CHRONIC + USMER +
#                       MEDICAL_UNIT * PATIENT_TYPE + OBESITY * CARDIOVASCULAR + 
#                       HIPERTENSION * INMSUPR + poly(COPD, 2) + I(RENAL_CHRONIC^2),  
#                    family=binomial(link = "logit"), 
#                    data = Train_COVID)
# fit_1 <- lm(SURVIVED_bin ~ SEX, Train_COVID)
# fit_2 <- lm(SURVIVED_bin ~ poly(SEX, 2), Train_COVID)
# fit_3 <- lm(SURVIVED_bin ~ poly(SEX, 3), Train_COVID)
# fit_4 <- lm(SURVIVED_bin ~ poly(SEX, 4), Train_COVID)
# fit_5 <- lm(SURVIVED_bin ~ poly(SEX, 5), Train_COVID)
# 
# table(Train_COVID$ASTHMA)
# #fit <- lm(SURVIVED_bin ~ poly(AGE_TRUNK, 7) + SEX + , Train_COVID)
# 
...
fit models with single variables and compare p-values by ANOVA. 
AGE_TRUNK has a p-value of 2.411e-09 in degree 7 and 0.3927 from degree 7 to degree 8, therefore AGE_TRUNK is sufficient in degree 7. 
SEX is binomial thus cannot fit as polynomial variable.
ASTHMA has three values: 1, 2 and 98. There is no definition of 98 in the column but here I treat it as missing data. 
~~~
...
Your code:
```{r,echo=TRUE}
size <- floor(dim(College)[1]*0.3)
set.seed(1257)
samp_idx <- sample(1:dim(College)[1], size, FALSE)
col_test <- College[samp_idx,]
col_train <- College[-samp_idx,]

col.regfit_full <- regsubsets(Outstate ~., nvmax = 12, col_train)
reg.summary <- summary(col.regfit_full)
reg.summary
reg.summary$rsq

col.fit_1 <- lm(Outstate ~ Private+Room.Board+perc.alumni+Expend+Grad.Rate+PhD,col_train)
summary(col.fit_1)
```
...
~~~
R-square statistic suggests  5 or 6 variables thus I choose subset of Private, Room.Board, perc.alumni, Expend, Grad.Rate, PhD.
...
# table(cut(col_test$Room.Board, 4))
# table(cut(col_test$Grad.Rate, 5))
# table(cut(col_test$PhD, 6))
# attr(bs(col_test$PhD, df = 7), "knots")
# table(cut(col_test$perc.alumni, 6))
# table(cut(col_test$Expend, 4))
# attr(bs(col_test$Expend, df = 6), "knots")
col.gam <- lm(Outstate ~ Private + ns(Room.Board, 4) + ns(perc.alumni, 6) +
                ns(Expend, knots = c(6742, 8420, 10912)) + 
                ns(Grad.Rate, 5) + ns(PhD, knots = c(60, 72, 80, 87.6)), col_test)
plot(col.gam, se = TRUE, col = "blue")
plot.Gam(col.gam, se = TRUE, col = "red")
```
...
auto.regfit_full <- regsubsets(mpg ~ cylinders+displacement+
                                 horsepower+weight+
                                 acceleration+year +origin , auto)
reg.summary <- summary(auto.regfit_full)
reg.summary
reg.summary$rsq
# The part runs well but cannot knit
# mames <- melt(auto[,c("cylinders","displacement", "horsepower",
#                           "weight" , "acceleration", "name", "year", "origin", "mpg")],
#               id.vars = "mpg")
...
auto.gam <- lm(mpg~ ns(cylinders, 4) + weight + ns(displacement, knots = c(119, 232)) + 
                 ns(horsepower, knots = c(84, 110)) + year + acceleration, auto)
plot.Gam(auto.gam, se = TRUE, col = "red")
summary(auto.gam)
```
